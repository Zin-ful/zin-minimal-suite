...
def download(client, name):
    ...
    with open(f"/etc/wireguard/{wg_title}{wg_number}.conf", "r") as file:
        ...
        if "y" in response:
            ...
            if "2" in choice:
                conf_file = f"#!!Keys={aval_pubkeys[selection]}&{aval_privkeys[selection]}:[Interface]\nPrivateKey = {aval_privkeys[selection]}\nAddress = {aval_ips[selection]}/24\n\nPostUp = iptables -P OUTPUT DROP\nPostUp = iptables -A OUTPUT -d {server_ip} -p udp --dport {server_port} -j ACCEPT\nPostUp = iptables -A OUTPUT -o wg0 -j ACCEPT\nPostUp = iptables -A OUTPUT -o lo -j ACCEPT\nPostUp = iptables -A INPUT -i wg0 -s 10.0.0.1 -j ACCEPT\nPostUp = iptables -A INPUT -i wg0 -j DROP\nPostUp = iptables -A OUTPUT -d 255.255.255.255 -j DROP\nPostUp = iptables -A OUTPUT -d 224.0.0.7 -j DROP\nPostUp = iptables -A OUTPUT -d 192.168.110.255 -j DROP\nPostUp = iptables -A OUTPUT -d 255.255.255.250 -j DROP\nPostUp = ip6tables -P OUTPUT DROP\nPostUp = ip6tables -A OUTPUT -o wg0 -j ACCEPT\nPostUp = ip6tables -A OUTPUT -o lo -j ACCEPT\nPostUp = ip6tables -A INPUT -i wg0 -s fd00::1 -j ACCEPT\nPostUp = ip6tables -A INPUT -i wg0 -j DROP\nPostUp = ip6tables -A OUTPUT -d ff00::/8 -j DROP\nPostUp = arptables -P OUTPUT ACCEPT\nPostUp = arptables -P INPUT ACCEPT\nPreDown = iptables -P OUTPUT ACCEPT\nPreDown = ip6tables -P OUTPUT ACCEPT\nPreDown = arptables -P OUTPUT ACCEPT\nPreDown = arptables -P INPUT ACCEPT\nPreDown = iptables -D OUTPUT -d {server_ip} -p udp --dport {server_port} -j ACCEPT\nPreDown = iptables -D OUTPUT -d 224.0.0.7 -j DROP\nPreDown = iptables -D OUTPUT -o wg0 -j ACCEPT\nPreDown = iptables -D OUTPUT -o lo -j ACCEPT\nPreDown = iptables -D INPUT -i wg0 -s 10.0.0.1 -j ACCEPT\nPreDown = iptables -D INPUT -i wg0 -j DROP\nPreDown = iptables -D OUTPUT -d 255.255.255.255 -j DROP\nPreDown = iptables -D OUTPUT -d 192.168.110.255 -j DROP\nPreDown = iptables -D OUTPUT -d 255.255.255.250 -j DROP\nPreDown = ip6tables -D OUTPUT -o wg0 -j ACCEPT\nPreDown = ip6tables -D OUTPUT -o lo -j ACCEPT\nPreDown = ip6tables -D INPUT -i wg0 -s fd00::1 -j ACCEPT\nPreDown = ip6tables -D INPUT -i wg0 -j DROP\nPreDown = ip6tables -D OUTPUT -d ff00::/8 -j DROP\n\n[Peer]\nPublicKey = {server_pubkey}\nAllowedIPs = 0.0.0.0/0, ::/0\nEndpoint = {server_ip}:{server_port}\nPersistentKeepalive = 25"
            else:
