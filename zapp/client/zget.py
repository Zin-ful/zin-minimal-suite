...
def app_get(server, name): #installs app
	...
	print(f"installing {name}, size is {file_size} bytes")
...
def list_local(*cmd): #lists local apps
...
def list_list(server, *arg):
	with open(f"{config_path}/apps_list.txt", "r") as file:
		apps = ""
		for i in file.readlines():
			apps += f"{i}\n"

cmd_list = {"listretr": app_listupd, "install": app_get, "reinstall": reinstall, "purge": remove, "update": update, "reconfigure": config, "list-apps": list_local, "list-aval": list_global, "list-upd": list_list,"help": helpy}		
