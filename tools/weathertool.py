...
def sort_list(text_list, top, middle, bottom, sortby):
    ...
    if not new_list:
        print(f"No {sortby}s found", colors["highlight"])
        return text_list
    return new_list
...
def find(alert_list, alert_details):
    print("Type in the phrase you want to search for:")
    screens["top"].refresh()
...
def get_alert(param):
    ...
    try:
        ...
        if not alert_data:
            print(f"No alerts for {parameters['state']}, YAYYYY :DDD")
            print(f"Shift+R to refresh")
            print(f"Shift+C for config")
            while True:
                if key == ord("R"):
                    return 0, 0, 0
                elif key == ord("C"):
                    ...
                    return archive()
        print(f"{len(alert_data)} Alerts for {parameters['state']}")
        print("Shift+H for help")
        clear_save()
